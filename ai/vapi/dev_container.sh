...
# Configuration
CONTAINER="${CONTAINER_NAME:-mb1}"

# IMPORTANT: allow callers to intentionally provide empty HOST_PORT /
# CONTAINER_PORT to force host networking (the script uses --network=host when
# either is empty). Use `${VAR-DEFAULT}` so default is only applied when VAR is
# truly *unset* (not when it is set to empty).
HOST_PORT="${HOST_PORT-5193}"
CONTAINER_PORT="${CONTAINER_PORT-5193}"
...
XAUTH_IN_CONTAINER="/root/.Xauthority"
...
build_image() {
    ...
    local dockerfile="${DOCKERFILE_PATH}/${DOCKERFILE_NAME}"
    ...
    if ! $DOCKER_CMD build \
        ...
        if ! $DOCKER_CMD build -f "${dockerfile}" -t "${CONTAINER}" "${DOCKERFILE_PATH}"; then
...
setup_x11() {
    ...
    # Normalize the XAUTH path: it must be a file on the host.
    # If it accidentally exists as a directory, Docker bind-mounts will fail.
    if [ -e "${XAUTH}" ] && [ ! -f "${XAUTH}" ]; then
        print_warning "${XAUTH} exists but is not a file; removing it to re-create as a file"
        sudo rm -rf "${XAUTH}" || true
    fi
...
get_x11_args() {
    ...
    if [ -n "${DISPLAY:-}" ]; then
        ...
        #fi
        if [ -f "${XAUTH}" ]; then
            # Mount the host xauth file into a stable in-container path.
            # Mounting it to the same /tmp path can break if the container-side
            # path exists as a directory.
            args+=" -v ${XAUTH}:${XAUTH_IN_CONTAINER}"
            args+=" -e XAUTHORITY=${XAUTH_IN_CONTAINER}"
        fi
...
start_container() {
    ...
    case "$status" in
        "running")
            ...
            # Ensure X11 mount prerequisites are sane even for already-running
            # containers (the host XAUTH path can drift between runs).
            setup_x11
            return 0
            ;;
        "stopped")
            print_info "Container ${CONTAINER} exists but is not running. Starting it..."
            # Ensure X11 mount prerequisites are sane before starting the existing container.
            # (The container may have been created with a /tmp/.docker.xauth bind mount.)
            setup_x11
            ...
            # Starting an existing container cannot change bind mount configuration.
            # If we get mount errors (file vs dir), the safest path is to recreate.
            print_warning "Failed to start existing container; recreating it..."
...
exec_shell() {
    ...
    # Normalize X11 auth file before attempting `docker exec`.
    # Some Docker setups will fail `docker exec` if a bind-mount source path
    # flipped between file/dir across runs (e.g. /tmp/.docker.xauth).
    setup_x11

    # If `docker exec` fails (commonly due to XAUTH mount issues), the most
    # reliable fix is to recreate the container since mount configuration is
    # immutable after creation.
    if ! $DOCKER_CMD exec "${CONTAINER}" /bin/sh -lc 'true' >/dev/null 2>&1; then
        print_warning "docker exec health-check failed; recreating container to recover from mount issues..."
...
# Cleanup on exit
# By default we DO NOT delete ${XAUTH}. Removing it can cause Docker to
# re-create it as a directory on subsequent runs, which then breaks bind mounts.
# If you want the previous behavior, set REMOVE_XAUTH_ON_EXIT=1.
cleanup() {
    if [ "${REMOVE_XAUTH_ON_EXIT:-0}" = "1" ] && [ -f "${XAUTH}" ]; then
